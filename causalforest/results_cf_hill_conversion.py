...
# Biến lưu trữ để vẽ biểu đồ cho Seed tốt nhất
...
for SEED in seeds:
    ...
    def objective(trial):
        # Tuning các tham số của chính Causal Forest
        cf_params = {
        ...
        cf_model = CausalForestWrapper(task_type='conversion', random_state=SEED, **cf_params)
    ...
    study = optuna.create_study(direction="maximize", study_name=f"CF_Conv_{SEED}", sampler=TPESampler(seed=SEED))
    study.optimize(objective, n_trials=30, show_progress_bar=True) # Giảm n_trials xuống 30 vì CF train khá lâu
    ...
    final_cf_model = CausalForestWrapper(task_type='conversion', random_state=SEED, **study.best_params)
    ...
    # Cập nhật best model để vẽ biểu đồ ở cuối
...
# Xuất CSV
csv_filename = "causal_forest_conversion_results.csv"
...
print(f"+) Causal_Forest_AUUC_Con: {best_auuc_score:.4f}")
print(f"+) Causal_Forest_AUQC_Con: {best_overall_auqc:.4f}")
print(f"+) Causal_Forest_Uplift@30_Con: {best_lift_score:.4f}")
print("="*50)

# Vẽ biểu đồ cho mô hình tốt nhất (Tự động gọi plot=True từ metrics.py)
