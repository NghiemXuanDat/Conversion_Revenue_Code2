...
def main():
    ...
    # 1. Tuning và huấn luyện S-Learner
    s_model = tune_and_train_s_learner(X_train, Y_train, T_train, X_val, Y_val, T_val, task_type='revenue')
    uplift_preds['S_learner'] = s_model.predict(X_test)

    # 2. Huấn luyện các Meta-Learners còn lại
    static_models = {
        'T_learner': get_t_learner('revenue'),
        'X_learner': get_x_learner('revenue'),
        'R_learner': get_r_learner('revenue'),
        'Causal_forest': get_causal_forest('revenue')
    }
    ...
    print("\nRevenue Uplift Modeling Results")
    model_names_map = {
        'S_learner': 'S_learner',
        'T_learner': 'T_learner',
        'X_learner': 'X_learner',
    ...
    for name, print_prefix in model_names_map.items():
        ...
        print(f"+) {print_prefix}_AUUC_Rev: {auuc:.4f}")
        print(f"+) {print_prefix}_AUQC_Rev: {auqc:.4f}")
        print(f"+) {print_prefix}_Uplift@30_Rev: {up30:.4f}")
    ...
    plt.title('Revenue Uplift: AUUC Curves', fontsize=14)
    plt.xlabel('Number of observations targeted')
    plt.ylabel('Cumulative Spend Uplift')
    plt.legend()
    plt.grid(True, linestyle=':', alpha=0.7)
    plt.tight_layout()
    plt.savefig('revenue_auuc_curve.png', dpi=300)
    plt.close()
    ...
    plt.title('Revenue Uplift: AUQC Curves', fontsize=14)
    plt.xlabel('Number of observations targeted')
    plt.ylabel('Cumulative Spend Qini')
    plt.legend()
    plt.grid(True, linestyle=':', alpha=0.7)
    plt.tight_layout()
    plt.savefig('revenue_auqc_curve.png', dpi=300)
    plt.close()

    print("\n+) Đã lưu thành công biểu đồ revenue_auuc_curve.png và revenue_auqc_curve.png")
