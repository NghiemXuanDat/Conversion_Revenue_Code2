# results_tlearner_hill_revenue.py
import pandas as pd
import optuna
...
# 2. Khởi tạo Pipeline cho Revenue (Sẽ tự động dùng LGBMRegressor)
pipeline = TLearnerPipeline(task_type='revenue')
...
csv_filename = "t_learner_revenue_results.csv"
results_df.to_csv(csv_filename, index=False)

# 5. Output hiển thị kết quả tốt nhất và vẽ biểu đồ theo format yêu cầu
print("\n" + "="*50)
print("Revenue Uplift Modeling Results")
print(f"+) T_learner_AUUC_Rev: {best_metrics['auuc']:.4f}")
print(f"+) T_learner_AUQC_Rev: {best_metrics['auqc']:.4f}")
print(f"+) T_learner_Uplift@30_Rev: {best_metrics['lift']:.4f}")
