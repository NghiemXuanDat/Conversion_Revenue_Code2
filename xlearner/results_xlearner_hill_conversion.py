# File: results_xlearner_hill_conversion.py
import pandas as pd
import numpy as np
import optuna
...
import sys
sys.path.append("../..") # Tuỳ chỉnh đường dẫn cho phù hợp với cấu trúc thư mục của bạn
...
for SEED in seeds:
    print(f"\n{'='*50}")
    print(f"🚀 CONVERSION UPLIFT - BẮT ĐẦU CHẠY VỚI SEED: {SEED}")
    print(f"{'='*50}")
    ...
    def objective(trial):
        ...
        # Lấy X-Learner cho bài toán conversion
        x_learner = get_xlearner(task_type='conversion', params=params)
        
        # EconML sử dụng cú pháp fit(Y, T, X)
        x_learner.fit(Y=y_train, T=t_train, X=X_train)
        
        # Lấy Uplift Score bằng hàm effect() và flatten mảng
    ...
    study = optuna.create_study(direction="maximize", study_name=f"X_Learner_Conv_{SEED}", sampler=fixed_sampler)
...
print(f"+) X_learner_AUUC_Con: {best_overall_auuc:.4f}")
print(f"+) X_learner_AUQC_Con: {best_overall_auqc:.4f}")
print(f"+) X_learner_Uplift@30_Con: {best_overall_lift:.4f}")
print("="*50)

print("💾 ĐANG LƯU KẾT QUẢ VÀO FILE CSV...")
results_df = pd.DataFrame(results)
csv_filename = "x_learner_conversion_results.csv"
